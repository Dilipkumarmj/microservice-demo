...
public class TwitterElasticQueryWebClient implements ElasticQueryWebClient {
    ...
    private WebClient.ResponseSpec getWebClient(ElasticQueryWebClientRequestModel requestModel){
        return webClientBuilder.build().method(HttpMethod.valueOf(elasticQueryWebClientConfigData.getQueryByText().getMethod()))
                ...
                .onStatus(httpStatus -> httpStatus.equals(HttpStatus.ACCEPTED.is4xxClientError()),
                        clientResponse -> Mono.just(new ElasticQueryWebClientException(clientResponse.statusCode().getReasonPhrase())))
                .onStatus(httpStatus -> httpStatus.equals(HttpStatus.ACCEPTED.is5xxServerError()),
                        clientResponse -> Mono.just(new ElasticQueryWebClientException(clientResponse.statusCode().getReasonPhrase())));
